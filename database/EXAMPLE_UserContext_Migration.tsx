 ...
 * MIGRATION NOTES:
 * 
 * 1. Removed Convex imports:
 *    - import { useQuery, useMutation } from 'convex/react'
 *    - import { api } from '../convex/_generated/api'
 * 
 * 2. Added PostgreSQL imports:
 *    - import { getUserByEmail, createOrGetUser, ... } from '../database'
 * 
 * 3. Replaced Convex hooks:
 *    - Before: const user = useQuery(api.Users.GetUser, { email })
 *    - After: useState + useEffect with getUserByEmail(email)
 * 
 * 4. Replaced mutations:
 *    - Before: const updateUser = useMutation(api.Users.updateTask)
 *    - After: async function updateUserProfile(data) { await updateUserProfileDB(...) }
 * 
 * 5. Added proper error handling and loading states
 * 
 * 6. Added refresh function to reload user data when needed
 * 
 * USAGE IN COMPONENTS:
 * 
 * Before (Convex):
 ...
 * await updateUser({ id: user._id, ... });
 * ```
 * 
 * After (PostgreSQL with Context):
 * ```
 * const { user, loading, updateUserProfile } = useUser();
 * await updateUserProfile({ weight: '75kg', ... });
 * ```
 */
